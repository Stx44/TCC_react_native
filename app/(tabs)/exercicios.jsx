...
export default function Exercicios() {
  ...
  // --- Lógica de Notificação (2 HORAS) ---
  const agendarLembrete = async (nomeExercicio) => {
    try {
      await Notifications.scheduleNotificationAsync({
        content: {
          title: "Foco nos Treinos e Metas! 🎯💪",
          body: `Lembrete: O exercício "${nomeExercicio}" e suas metas semanais precisam de atenção. Vamos concluir?`,
          data: { url: '/notic' },
        },
        trigger: {
          seconds: 2 * 60 * 60, // 2 Horas (em segundos)
        },
      });
    } catch (error) {
  ...
  const salvarEvento = async () => {
    ...
    // 1. Agenda a notificação para daqui a 2 horas
    await agendarLembrete(novoEvento);
    ...
    // Removido o Alert de teste. O salvamento ocorre silenciosamente agora.
  };
